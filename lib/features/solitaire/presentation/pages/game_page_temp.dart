            ...
            Expanded(
              child: Stack(
                children: [
                  ...
                  ..._buildPlaceholders(layout, gameState, gameController),
    ...
    return [
      ...
      ...layout.foundationRects.asMap().entries.map((entry) {
        ...
        return Positioned.fromRect(
          ...
          child: pile_widgets.PileDropTarget(
            onAccept: (game.Card card) {
              print(r'[GamePage] Accepting card on foundation $index');
      ...
      ...layout.tableauColumnRects.asMap().entries.map((entry) {
        ...
        return Positioned.fromRect(
          ...
          child: pile_widgets.PileDropTarget(
            onAccept: (game.Card card) {
              print(r'[GamePage] Accepting card on tableau $index');
  ...
  List<Widget> _buildPositionedCards(
    ...
    if (gameState.stock.isNotEmpty) {
      cards.add(
        _buildCard(
          ...
          layout,
          key: const ValueKey(r'stock_${gameState.stock.topCard!.id}'),
    ...
    for (var i = 0; i < gameState.waste.cards.length; i++) {
      ...
      cards.add(
        _buildCard(
          ...
          card,
          layout,
          key: const ValueKey(r'waste_${card.id}'),
          isDraggable: i ==
      ...
      if (foundation.isNotEmpty) {
        ...
        cards.add(
          _buildCard(
            ...
            card,
            layout,
            key: const ValueKey(r'foundation_${foundationIndex}_${card.id}'),
            isDraggable: true,
          ),
        );
      }
    ...
    for (var columnIndex = 0;
      ...
      for (var cardIndex = 0; cardIndex < pile.cards.length; cardIndex++) {
        ...
        final position = layout.cardOffsetInTableau(columnIndex, cardIndex,
            faceUp: card.faceUp);
        cards.add(
          _buildCard(
            position,
            card,
            layout,
            key: const ValueKey(r'tableau_${columnIndex}_${card.id}'),
            isDraggable: card.faceUp,
            onTap: card.faceUp
                ? () => gameController.tapCard(card,
                    source: 'tableau', sourceIndex: columnIndex)
                : null,
          ),
        );
      }
    ...
    return AnimatedPositioned(
      ...
      child: CardView(
        ...
        onDragStarted: isDraggable
            ? () =>
                print(r'[GamePage] Drag started: ${card.rank} of ${card.suit}')
            : null,
        onDragCompleted: isDraggable
            ? () => print(
                r'[GamePage] Drag completed: ${card.rank} of ${card.suit}')
            : null,
