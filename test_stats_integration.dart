...
/// Test simple d'intégration pour les statistiques
void main() async {
  WidgetsFlutterBinding.ensureInitialized();

  // Initialiser Hive avec un répertoire temporaire
  await Hive.initFlutter('test_stats');
  ...
  print('=== Test du système de statistiques ===');
  ...
  // Tester les filtres
  await _testFilters(statsRepo);

  // Tester les tendances
  await _testTrends(statsRepo);
...
Future<void> _testFilters(GameStatsRepository repo) async {
  print('\n🔍 Test des filtres...');
...
Future<void> _testTrends(GameStatsRepository repo) async {
  print('\n📈 Test des tendances...');

  final sessions = await repo.query();

  print('  - Nombre de sessions récupérées: ${sessions.length}');
  assert(sessions.isNotEmpty, 'Devrait y avoir des sessions');

  for (final session in sessions.take(3)) {
    print(
        '  - ${session.startedAt.toIso8601String().substring(0, 10)}: ${session.won ? 'Gagné' : 'Perdu'}');
  }
